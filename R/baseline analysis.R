# ESG and Banking Financial Metrics Analysis
# Based on EBA supervisory data 2020-2023
...
# Load the dataset (assuming it's called data_analysis)
# data_analysis <- read.csv("your_data_file.csv")

# Create indicators for years
data_analysis$year_factor <- as.factor(data_analysis$year)

# Handle missing values - following Apergis et al. (2022)
# Exclude observations with missing values in key variables
data_clean <- data_analysis %>%
  filter(!is.na(esg_score) & 
           !is.na(total_assets) &
           !is.na(common_equity_tier_1_as_a_percentage_of_risk_exposure_amount_transitional_definition))

# Create size variable (natural log of total assets)
data_clean$log_total_assets <- log(data_clean$total_assets)

# Create profitability proxy - since direct profitability measures aren't in the column list,
# we'll need to calculate or use appropriate proxies based on available data

# Create risk-weighted asset ratio
data_clean$rwa_ratio <- data_clean$total_risk_exposure_amount / data_clean$total_assets

# 2. DESCRIPTIVE STATISTICS --------------------------------------------------

# Create ESG categories for descriptive analysis (low, medium, high)
data_clean <- data_clean %>%
  mutate(esg_category = cut(esg_score, 
...
summary_stats <- data_clean %>%
  ...
  summarise(
    n = n(),
    # Capital ratios
    cet1_ratio_mean = mean(common_equity_tier_1_as_a_percentage_of_risk_exposure_amount_transitional_definition, na.rm = TRUE),
    cet1_ratio_sd = sd(common_equity_tier_1_as_a_percentage_of_risk_exposure_amount_transitional_definition, na.rm = TRUE),
    tier1_ratio_mean = mean(tier_1_as_a_percentage_of_risk_exposure_amount_transitional_definition, na.rm = TRUE),
    tier1_ratio_sd = sd(tier_1_as_a_percentage_of_risk_exposure_amount_transitional_definition, na.rm = TRUE),
    total_capital_ratio_mean = mean(total_capital_as_a_percentage_of_risk_exposure_amount_transitional_definition, na.rm = TRUE),
    total_capital_ratio_sd = sd(total_capital_as_a_percentage_of_risk_exposure_amount_transitional_definition, na.rm = TRUE),
    
    # Leverage
    leverage_ratio_mean = mean(leverage_ratio_using_a_transitional_definition_of_tier_1_capital, na.rm = TRUE),
    leverage_ratio_sd = sd(leverage_ratio_using_a_transitional_definition_of_tier_1_capital, na.rm = TRUE),
    ...
    # Size
    log_total_assets_mean = mean(log_total_assets, na.rm = TRUE),
    log_total_assets_sd = sd(log_total_assets, na.rm = TRUE)
  )
