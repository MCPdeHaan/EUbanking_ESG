...
```{r}
# Run exploratory analysis with the fixed functions
exploratory_results <- run_exploratory_analysis(data_analysis)
# Create formatted report with proper table formatting
exploratory_report <- create_exploratory_report(exploratory_results)
```


```{r summary-statistics-table, results='asis'}
if(!is.null(exploratory_report$summary_statistics)) {
  cat(exploratory_report$summary_statistics)
}
```

```{r exploratory-data-overview, results='asis'}
if(!is.null(exploratory_report$data_coverage)) {
  cat(exploratory_report$data_coverage)
}
```

```{r exploratory-country-composition, results='asis'}
if(!is.null(exploratory_report$country_composition)) {
  cat(exploratory_report$country_composition)
}
```

```{r exploratory-yearly-esg, results='asis'}
if(!is.null(exploratory_report$yearly_esg)) {
  cat(exploratory_report$yearly_esg)
}
```

```{r exploratory-esg-by-size, results='asis'}
if(!is.null(exploratory_report$esg_by_size)) {
  cat(exploratory_report$esg_by_size)
}
```
...
```{r exploratory-esg-hist, fig.height=5, fig.width=10}
# Display ESG histogram plot
if(!is.null(exploratory_results$esg_hist)) {
  exploratory_results$esg_hist
}
```

```{r exploratory-pillar-boxplot, fig.height=5, fig.width=10}
# Display ESG pillar boxplot
if(!is.null(exploratory_results$pillar_boxplot)) {
  exploratory_results$pillar_boxplot
}
```
...
```{r exploratory-esg-trend, fig.height=5, fig.width=10}
# Display ESG trend plot
if(!is.null(exploratory_results$esg_trend_plot)) {
  exploratory_results$esg_trend_plot
}
```

```{r exploratory-pillar-trend, fig.height=5, fig.width=10}
# Display ESG pillar trends
if(!is.null(exploratory_results$pillar_trend_plot)) {
  exploratory_results$pillar_trend_plot
}
```
