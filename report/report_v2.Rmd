...
### Overall ESG Score Effects

Table \@ref(fig:time-trends) presents the results of the two-way fixed effects models examining the relationship between banks' overall ESG scores and various risk measures. These models control for both time-invariant bank characteristics and common time trends, addressing concerns about unobserved heterogeneity as outlined in the methodology.

The results reveal several notable patterns. First, the coefficient of the overall ESG score on CET1 risk exposure is 0.000, with a standard error of 0.005, indicating no statistically significant linear relationship between overall ESG performance and this fundamental capital adequacy measure. Similar patterns emerge for tier1_risk_exposure (coefficient = 0.005, SE = 0.006) and totalcap_risk_exposure (coefficient = 0.006, SE = 0.006), neither of which reaches conventional significance levels. These findings suggest that the direct linear impact of overall ESG scores on regulatory capital ratios is minimal in the European banking sector during the 2020-2023 period.
For leverage_ratio, the ESG score coefficient is negative (-0.001) but statistically insignificant, while the provisions variable shows a large positive coefficient (116.065) with a substantial standard error (182.282), again failing to reach statistical significance. The provisions_ratio shows a positive but insignificant relationship (coefficient = 0.003, SE = 0.004). The liquidity_ratio and rwa_ratio both show negative relationships with ESG scores (-0.003 and -0.005 respectively), though neither reaches statistical significance.
The quadratic terms (ESG Score²) are consistently close to zero across all risk measures, with none showing statistical significance. This suggests an absence of strong non-linear relationships between overall ESG performance and risk metrics in the sample. The lack of significance in both linear and quadratic terms indicates that the relationship between aggregate ESG scores and risk measures may be more complex than can be captured by these specifications alone, or that heterogeneity in the sample is masking potential effects.

Regarding control variables, bank size (Log Assets) shows consistently strong and statistically significant associations with risk measures. The positive coefficients for CET1, tier1, and total capital risk exposures (0.194***, 0.185***, and 0.238*** respectively) indicate that larger banks tend to maintain higher regulatory capital ratios, potentially reflecting their systemic importance and corresponding regulatory expectations. The equity-to-assets ratio similarly demonstrates strong positive associations with most risk measures, particularly with capital adequacy metrics (coefficients ranging from 1.327** to 4.727***), confirming the mechanical relationship between equity capital and regulatory risk measures.

The R² values, ranging from 0.142 (liquidity_ratio) to 0.487 (totalcap_risk_exposure), indicate moderate explanatory power for these models. The relatively higher R² for capital-related risk measures suggests that the included variables capture capital adequacy determinants more effectively than liquidity or provision factors.
