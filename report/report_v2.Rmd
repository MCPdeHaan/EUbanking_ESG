...
Disaggregating ESG into its component pillars reveals even more nuanced relationships that vary across the risk distribution, as shown in Figures \@ref(fig:quantile-environmental), \@ref(fig:qunatile-social), and \@ref(fig:quantile-governance).
...
Our quantile regression findings both complement and extend previous research on the relationship between ESG performance and banking risk. The negative association between social pillar scores and capital risk measures across multiple quantiles aligns with studies by Amiraslani et al. (2017) and Izcan & Bektas (2022), who found that social factors can enhance bank stability. However, our quantile approach reveals that this relationship is not uniform across the risk distribution, being particularly strong for banks in the middle to upper quantiles.

Similarly, the varied relationships observed for the environmental and governance pillars across different quantiles help explain some of the mixed findings in previous literature. For instance, Bannier et al. (2021) found that environmental aspects were negatively related to credit risk for U.S. firms but not consistently for European firms. Our quantile results suggest that such inconsistencies might partially result from heterogeneous effects across different segments of the risk distribution, which would be averaged out in conventional regression approaches.

The minimal relationship between ESG factors and leverage ratio across most quantiles aligns with findings from Palmieri et al. (2023), who documented limited direct associations between ESG performance and leverage decisions. However, our identification of a significant negative relationship between environmental scores and leverage ratio at the 0.25 quantile represents a novel finding that warrants further investigation.

### Limitations and Robustness Considerations

While quantile regression provides valuable insights into heterogeneous ESG effects across the risk distribution, several limitations should be acknowledged. First, the relatively small sample size (132 bank-year observations) limits statistical power, particularly at extreme quantiles where estimates become more uncertain. This is reflected in the wider confidence intervals at the 0.1 and 0.9 quantiles for many coefficients.

Second, while our models control for bank size and equity-to-assets ratio, they may not capture all relevant factors that influence the relationship between ESG performance and banking risk. Omitted variables that vary across the risk distribution could potentially bias our quantile estimates.

Third, the quantile regression approach employed here does not address potential endogeneity concerns, such as reverse causality or selection effects. Banks with particular risk profiles might strategically adjust their ESG investments, creating bidirectional relationships that our models do not disentangle.

To address some of these concerns, we implemented bootstrap methods for computing standard errors, as recommended by Koenker and Hallock (2001). This approach provides more robust inference in the presence of heteroskedasticity, which is likely present across different portions of the risk distribution.

## Principal Component Analysis

Building on the methodological framework outlined in Section 5.5, this section presents the results from the Principal Component Analysis (PCA) of ESG dimensions and their relationship with banking risk measures. While the previous fixed effects and quantile regression analyses provide valuable insights into the relationships between aggregate ESG scores or individual pillars and risk metrics, PCA offers a complementary approach that addresses potential multicollinearity and captures the underlying structure of ESG performance beyond the conventional three-pillar framework.

### Component Structure and Variance Explanation

Figure \@ref(fig:pca-results) presents the scree plot of principal components, revealing that the first five components collectively explain approximately 83% of the total variance in ESG variables. The first component alone accounts for 46.2% of the variance, followed by the second component with 13.8%, indicating a substantial dimensionality reduction from the original set of ESG variables. This pattern suggests that while ESG performance is multidimensional, a relatively small number of underlying factors capture the majority of variation across European banks.
...
### Relationship Between Principal Components and Banking Risk
...
Table \@ref(tab:pca-comparison-table) provides a direct comparison of model performance between the original ESG-based models and the PCA-based models across different risk measures. This comparison offers insights into whether the dimensionality reduction and multicollinearity mitigation provided by PCA enhance the explanatory power of our models.

```{r pca-comparison-table}
pca_results$visualizations$comparison_table
```

The results indicate that the PCA-based models generally outperform the original models in terms of R-squared values. For CET1 risk exposure, the R-squared increases from 0.444 to 0.447, representing a modest improvement of 0.003. Similar small improvements are observed for Tier 1 risk exposure (0.425 to 0.427) and total capital risk exposure (0.445 to 0.447). More substantial gains emerge for leverage ratio (0.364 to 0.391, an improvement of 0.027), provisions (0.179 to 0.196, an improvement of 0.017), provisions ratio (0.187 to 0.211, an improvement of 0.024), and RWA ratio (0.193 to 0.219, an improvement of 0.026).

The only measure for which the PCA approach does not improve model fit is liquidity ratio, where the R-squared slightly decreases from 0.143 to 0.140. This exception might reflect the unique determinants of bank liquidity, which may be less closely connected to the underlying dimensions of ESG performance captured by our principal components.

The general pattern of improved model performance suggests that the PCA approach effectively captures important aspects of ESG performance that are relevant for banking risk, while reducing noise and redundancy in the original ESG variables. This aligns with methodological insights from Goss and Roberts (2010), who similarly found that dimensionality reduction techniques enhanced the explanatory power of models examining the relationship between social responsibility and financial outcomes.

### Interpretation and Implications

The PCA results offer several important insights beyond those provided by the aggregate ESG and pillar-level analyses. First, they confirm that ESG performance is multidimensional, with several distinct underlying factors that exhibit different relationships with banking risk measures. This complexity underscores the limitations of relying solely on aggregate ESG scores or even the three conventional pillars when examining the implications of sustainability for financial institutions.
...
Third, the improved model performance achieved through PCA suggests that this approach captures meaningful variation in sustainability performance that is relevant for banking risk. The gains in explanatory power, while modest for some risk measures, are consistent across multiple outcomes, indicating that the dimensionality reduction enhances our ability to identify associations between ESG factors and risk metrics.

The results also align with recent regulatory perspectives emphasizing the need for a more nuanced understanding of ESG risks in banking supervision. As noted by the European Banking Authority (2020), ESG factors can materialize in different ways across financial institutions, affecting various risk dimensions depending on business models and strategic priorities. The PCA approach, by identifying distinct underlying dimensions of ESG performance beyond conventional categories, offers a potentially valuable tool for supervisors seeking to assess how sustainability considerations might influence banking stability.

### Limitations of the PCA Approach

While the PCA results provide valuable insights, several limitations warrant consideration. First, the interpretation of principal components is inherently subjective, relying on patterns of variable loadings that may have multiple plausible explanations. While our interpretations align with theoretical expectations from the literature, alternative frameworks might suggest different meanings for the identified components.

Second, PCA assumes linear relationships between variables, potentially missing more complex interactions or non-linear patterns in ESG performance. This limitation is particularly relevant given the evidence of non-linear relationships between individual ESG pillars and risk measures identified in our fixed effects models (Section 6.3).

Third, the PCA approach, while addressing multicollinearity, does not resolve potential endogeneity concerns such as reverse causality or omitted variables. Banks' risk profiles might influence their sustainability priorities, creating bidirectional relationships that our models do not fully disentangle.

Finally, the relatively small sample size (132 bank-year observations) limits the stability and precision of our PCA results. With a larger set of ESG variables, more robust component structures might emerge, potentially capturing additional nuances in sustainability performance relevant for banking risk.

Despite these limitations, the PCA approach provides a valuable complement to the aggregate ESG and pillar-level analyses, offering insights into the underlying structure of sustainability performance and its implications for banking risk that would be difficult to obtain through conventional methods alone.
