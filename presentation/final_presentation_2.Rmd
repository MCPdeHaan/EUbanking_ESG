...
<div>
```{r, fig.height=3.0, fig.width=5.0}
...
- Addresses four key methodological challenges:
...
- Optimizes statistical power with 132 observations
...
- **Methodological Implication:** 
  Need for complementary analytical approaches
</div>
</div>
...
## PCA Results: Revealing ESG Dimensionality {.flexbox .vcenter}
...
## IV vs Fixed Effects: Tackling Endogeneity {.flexbox .vcenter}

<div class="columns-2">
<div>
```{r}
# Create a simpler table of IV vs FE results
iv_data <- data.frame(
  Measure = c("CET1 Risk", "Tier 1 Risk", "Total Cap Risk", "Leverage"),
  FE = c(0.001, 0.001, 0.001, -0.000),
  IV = c(0.007, 0.011, 0.015, -0.002),
  Difference = c("+600%", "+1000%", "+1400%", "larger")
)
...
- **Substantial Endogeneity Detected:**
  - IV estimates up to 14x larger than fixed effects
...
## Model Performance Enhancement {.flexbox .vcenter}

<div class="columns-2">
<div>
```{r}
# Create a simpler model comparison table with fewer rows
model_comp_data <- data.frame(
  Measure = c("CET1 Risk", "Leverage Ratio", "Provisions Ratio", "RWA Ratio"),
  Original = c(0.444, 0.364, 0.187, 0.193),
  PCA = c(0.447, 0.391, 0.211, 0.219),
  Improvement = c(0.003, 0.027, 0.024, 0.026)
)
...
<div style="margin-top: 30px; font-style: italic; font-size: 18px;">
